...
def sleep_wake_per_minute(subjects, norm='baseline'):
    ...
    for subj in subjects.keys():
        ...
        for chan in top_chans:
            ...
            # stim blocks, start after row 0 cause its the baseline
...
def sleep_wake_per_minute_avg_subj(subjects, norm='baseline'):
    blocks_dict = {'stim_w': [], 'stim_s': [], 'stim_t': [], 'undisturbed_w': [], 'undisturbed_s': [], 'undisturbed_t': []}
    for subj in subjects.keys():
        top_chans = subjects[subj]
        sleep_percent = pd.read_csv(f'results/{subj}_sleep_percent.csv')
        blocks_dict_subj = {'stim_w': [], 'stim_s': [], 'stim_t': [], 'undisturbed_w': [], 'undisturbed_s': [],
                       'undisturbed_t': []}
        for chan in top_chans:
            chan_rates = pd.read_csv(f'results\\{subj}_{chan}_rates.csv', index_col=0)
            baseline = chan_rates['rate'][0]
            ...
            for i in range(1, len(chan_rates['rate']) - 1, 2):
                ...
                if norm == 'baseline':
                    ...
                    y_axis = zscore(y_axis)
                if sleep_percent['sleep_percent'][i] == 100:
                    blocks_dict_subj['stim_s'].append(y_axis)
                elif sleep_percent['sleep_percent'][i] == 0:
                    blocks_dict_subj['stim_w'].append(y_axis)
                else:
                    blocks_dict_subj['stim_t'].append(y_axis)
            ...
            for i in range(2, len(chan_rates['rate']), 2):
                ...
                if norm == 'baseline':
                    ...
                    y_axis = zscore(y_axis)
                if sleep_percent['sleep_percent'][i] == 100:
                    blocks_dict_subj['undisturbed_s'].append(y_axis)
                elif sleep_percent['sleep_percent'][i] == 0:
                    blocks_dict_subj['undisturbed_w'].append(y_axis)
                else:
                    blocks_dict_subj['undisturbed_t'].append(y_axis)

        for key in blocks_dict.keys():
            if len(blocks_dict_subj[key]) > 0:
                avg_df = pd.DataFrame(blocks_dict_subj[key], columns=['0', '1', '2', '3', '4', '5'])
                means = [avg_df[str(i)].mean() for i in range(0, 6)]
                blocks_dict[key].append(means)
    return blocks_dict


def plot_all_vigilance(subjects, norm='baseline', avg_subj=False):
    plt.rcParams['figure.figsize'] = [8, 5]
    blocks_dict = sleep_wake_per_minute_avg_subj(subjects, norm=norm) if avg_subj else sleep_wake_per_minute(subjects, norm=norm)
...
def sleep_wake_blocks(subjects, norm='baseline', violin=True):
    baselines = []
    blocks_dict = {'stim_w': [], 'stim_s': [], 'stim_t': [], 'undisturbed_w': [], 'undisturbed_s': [], 'undisturbed_t': []}
    colors = {'stim_w': 'tab:blue', 'stim_s': 'tab:orange', 'stim_t': 'tab:green', 'undisturbed_w': 'tab:red',
              'undisturbed_s': 'tab:purple', 'undisturbed_t': 'tab:brown'}
    ...
    if violin:
        violin_data = []
        for block_type in blocks_dict.keys():
            violin_data.append([x[1] for x in blocks_dict[block_type]])
        v = plt.violinplot(violin_data, showmeans=True, showmedians=True)
        v['cmeans'].set_color('b')
        v['cmedians'].set_color('black')
        for i, body in enumerate(v["bodies"]):
            body.set_facecolor(colors[list(blocks_dict.keys())[i]])
            body.set_edgecolor('black')
            body.set_alpha(0.8)
        plt.xticks([1, 2, 3, 4, 5, 6], blocks_dict.keys(), rotation=45)
    else:
        for block_type in blocks_dict.keys():
            blocks_num = len(blocks_dict[block_type])
            if blocks_num > 0:
...
def sleep_wake_blocks_avg_subj(subjects, norm='baseline'):
    baselines = []
    blocks_dict = {'stim_w': [], 'stim_s': [], 'stim_t': [], 'undisturbed_w': [], 'undisturbed_s': [], 'undisturbed_t': []}
    colors = {'stim_w': 'tab:blue', 'stim_s': 'tab:orange', 'stim_t': 'tab:green', 'undisturbed_w': 'tab:red',
              'undisturbed_s': 'tab:purple', 'undisturbed_t': 'tab:brown'}
    for subj in subjects.keys():
        top_chans = subjects[subj]
        sleep_percent = pd.read_csv(f'results/{subj}_sleep_percent.csv')
        blocks_dict_subj = {'stim_w': [], 'stim_s': [], 'stim_t': [], 'undisturbed_w': [], 'undisturbed_s': [], 'undisturbed_t': []}
        for chan in top_chans:
            chan_rates = pd.read_csv(f'results\\{subj}_{chan}_rates.csv', index_col=0)
            baseline = chan_rates['rate'][0]
            baselines.append(baseline)
        ...
        for key in blocks_dict.keys():
            if len(blocks_dict_subj[key]) > 0:
                blocks_dict[key].append(np.array(blocks_dict_subj[key])[:, 1].mean())

    for block_type in blocks_dict.keys():
        blocks_num = len(blocks_dict[block_type])
        if blocks_num > 0:
            # avg_df = pd.DataFrame(blocks_dict[block_type], columns=['baseline', 'block avg'])
            mean = np.array(blocks_dict[block_type]).mean()
            std = sem(blocks_dict[block_type], nan_policy='omit') if blocks_num > 1 else None
            # plt.errorbar(['baseline', 'block avg'], means, yerr=stds, capsize=5, fmt='-o',
            #              label=f'{block_type}-{str(blocks_num)} blocks')
            rect = plt.bar([block_type], mean, yerr=std, capsize=5, alpha=0.7, color=colors[block_type])
            # plt.bar_label(rect, [f'n={blocks_num}'], padding=1)
            plt.bar_label(rect, [f'avg={round(mean, 1)}'], padding=1)
            plt.scatter([block_type] * len(blocks_dict[block_type]), blocks_dict[block_type], color=colors[block_type])
        else:
            plt.bar([block_type], 0)
    plt.xticks(rotation=45, ha='right')
...
# sleep_wake_blocks_avg_subj(manual_select_14_thresh)
plot_all_vigilance(manual_select_14_thresh, avg_subj=True)
...
# sleep_wake_blocks_avg_subj(curr_subjects)
