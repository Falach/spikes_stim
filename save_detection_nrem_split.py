...
def detect_subj_chan(subj, channels):
    ...
    nrem_epochs, nrem_stim_epochs, stim = get_nrem_epochs(subj)
    stim_start = stim[0][0]
    stim_end = stim[-1][1]
    ...
    for i, (start, end) in enumerate(nrem_epochs[:]):
        ...
        if start < stim_start and end < stim_start:
            ...
            if end > stim_end:
                ...
                start2 = stim_start
                j = i
                # might be more than one stim session in the epoch
                while end2 < stim_end:
            ...
            j = i
            while end2 < stim_end:
                rates = fill_row(remove_stim(subj, raw.copy().crop(tmin=start2, tmax=end2), start2, end2), rates,
                                 is_stim=1)
                j += 1
                start2, end2 = nrem_epochs[j]
    ...
    results_df.to_csv(f'results/{subj}_{channels[0]}_nrem_split_rates.csv')
