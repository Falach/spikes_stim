...
import joblib
from utils import sr, remove_stim, get_stim_starts, edf_path, stim_path, get_clean_channels, get_nrem_epochs, is_stim_in_epoch
...
def detect_subj_chan(subj, channels):
    ...
    nrem_epochs, nrem_stim_epochs = get_nrem_epochs(subj)
    raw = mne.io.read_raw_edf(edf_path % (subj, subj)).pick_channels(channels)
    # until first nrem
    raw_without_stim = remove_stim(subj, raw.copy().crop(tmin=0, tmax=nrem_stim_epochs[0][0]), 0, nrem_stim_epochs[0][0])
    rates = fill_row(raw_without_stim, rates, is_stim=is_stim_in_epoch(subj, 0, nrem_stim_epochs[0][0]))
    ...
    for i, (start, end) in enumerate(nrem_epochs):
        ...
        rates = fill_row(raw_without_stim, rates, is_stim=[start, end] in nrem_stim_epochs)
        if i + 1 < len(nrem_epochs):
            # the next sleep stage is the time between the end of the curr section and the start of the next
            rates = fill_row(raw.copy().crop(tmin=end, tmax=nrem_epochs[i + 1][0]),
                             rates, is_stim=is_stim_in_epoch(subj, end, nrem_epochs[i + 1][0]))
